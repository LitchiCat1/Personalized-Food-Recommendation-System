...
def predict_vision_food():
    ...
    try:
        parsed = call_gemini_food_recognition_with_rotation(image_base64, mime_type, api_keys, mode=mode)
        return jsonify(
            build_vision_food_response(
                parsed,
                storage,
...
def add_record():
    ...
    record = {
        ...
        "total_calcium": data.get("total_calcium", 0),
        "total_iron": data.get("total_iron", 0),
