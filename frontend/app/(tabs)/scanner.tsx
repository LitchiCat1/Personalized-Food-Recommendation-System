...
type ScanMode = 'camera' | 'gallery' | 'menu' | 'label' | 'manual';

const SCAN_MODE_OPTIONS = [
  { value: 'camera', label: '拍照' },
  { value: 'gallery', label: '相簿' },
  { value: 'menu', label: '拍菜單' },
...
export default function ScannerScreen() {
  ...
  const modeConfig = {
    ...
    gallery: { icon: 'images-outline' as const, title: '從相簿選擇餐點照片', hint: '可使用已拍攝的餐點照片，不需要重新開啟相機。', action: '選擇餐點照片', onPress: handleGallery },
    menu: { icon: 'restaurant-outline' as const, title: '拍攝或上傳菜單照片', hint: '上傳餐廳菜單或價目表照片，AI 會自動解析可選餐點與安全評估。', action: '選擇菜單照片', onPress: handleGallery },
  ...
  const hasAnyResult = results.length > 0 || ocrDraft !== null || manualResults.length > 0;

  const resultWorkspace = hasAnyResult ? (
