...
export default function DashboardScreen() {
  ...
  const { calories, protein, carbs, fat, sodium, fiber, calcium, iron } = dailyNutrition;
  ...
  const nutritionContent = (
    <SectionBlock title="營養素進度" subtitle="依每日目標檢查下一餐需要補足或控制的項目。">
      {sensitiveConditions.length ? (
        <View style={styles.sensitivityNotice}>
          ...
          <View style={styles.sensitivityCopy}>
            <Text style={styles.sensitivityTitle}>疾病目標已動態套用</Text>
            <Text style={styles.sensitivityText}>已依{sensitiveConditions.join('、')}條件自動調整鈉/碳水/蛋白/鈣/鐵上限或下限。</Text>
          </View>
        </View>
      ) : null}
      <View style={styles.nutrientStack}>
        ...
        {calcium ? <NutrientBar label={calcium.label} current={calcium.current} target={calcium.target} unit={calcium.unit} color={calcium.color} attentionLabel={getAttentionLabel('calcium')} /> : null}
        {iron ? <NutrientBar label={iron.label} current={iron.current} target={iron.target} unit={iron.unit} color={iron.color} attentionLabel={getAttentionLabel('iron')} /> : null}
      </View>
    </SectionBlock>
  );
