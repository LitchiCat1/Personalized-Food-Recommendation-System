...
- 修正菜單圖片辨識改走 `/restaurant/menu`，並在完成、無結果與失敗時顯示明確狀態。
- 新增需登入的 `/barcode/<barcode>` Open Food Facts 查詢 API，補上條碼查詢按鈕與營養資料映射。
- 條碼、菜單結果均標示為外部資料或估算，提醒使用者以包裝／店家現場標示為準。
...
- 修正店家 AI 摘要遇到 Gemini 非嚴格 JSON 時顯示 `Expecting value` 解析例外；現在使用結構化 JSON schema、兼容常見格式偏差並嘗試下一組模型，全部失敗時回傳低信心且明確標示不可用的摘要。
- 補強 Gemini 結構化回應讀取（同時支援 `text` 與結構化物件欄位），並加入不含敏感資料的回應形狀診斷，避免模型回應異常時只能看到模糊錯誤。
- 提高店家摘要的 Gemini 輸出上限，避免中文推薦理由被 `MAX_TOKENS` 截斷而誤判為無法使用。
