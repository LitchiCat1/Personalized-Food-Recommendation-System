...
def call_gemini_food_recognition_with_rotation(image_b64: str, mime_type: str, api_keys: list[str], mode: str = "food") -> dict:
    if not api_keys:
    ...
    for key_index, api_key in enumerate(api_keys):
        for model in models:
            attempt += 1
            try:
                return call_gemini_food_recognition(image_b64, mime_type, api_key, model, mode)
...
def call_gemini_food_recognition(image_b64: str, mime_type: str, api_key: str, gemini_model: str | None = None, mode: str = "food") -> dict:
    gemini_model = gemini_model or get_gemini_models()[0]
    if mode == "menu":
        prompt = (
            "你是台灣飲食紀錄 App 的菜單與價目表辨識器。"
            "請辨識這張餐廳菜單、價目表或餐牌照片中的所有菜色/餐點項目名稱與單份估計份量。"
            "特別注意台灣常見餐點，例如排骨飯、牛肉麵、貢丸湯、燙青菜、珍奶。"
            "你不能連線資料庫，也不要直接產生營養數字；營養資料會由後端用你提供的候選名稱查 TFDA 資料庫。"
            "請提供繁體中文食品名稱與同義候選。"
            "只回傳合法 JSON，不要加 markdown、不要加解釋。"
            "JSON schema: "
            '{"meal_guess":"菜單照片辨識","items":[{"name_zh":"","name_en":"",'
            '"confidence":0.0,"estimated_weight_g":100.0,"portion_description":"單份",'
